...
public class caseTwo
{
    ...
    public caseTwo(Job[] job, Segment[] mem)
    {
        ...
        try
        {
            w = new BufferedWriter(new FileWriter("JScaseTwoOut.txt")); //create the output file
            for(int i = 0; i < timeUnits; i++)
            {
                ...
                //Loop through the memory segments and assign those that are ready to the ready queue
                for(int n = 0; n < mem.length; n++)
                {
                    if(mem[n].getInUse() && mem[n].getJob().getQue() == false)
                    { //if current memory segment has a job and that job isn't in the queue
                        mem[n].getJob().setQue(true); //set its value as true
                        rQueue.add(mem[n].getJob()); //add it into the queue
                    }
                }
                if(rQueue.size()<=4)
				...
				for(int q = 0; q < mem.length; q++)
				{ //loops through all the memory segments
					if(mem[q].getInUse() == false) //if the memory segment is not in use
						memoryLoss += mem[q].getSize(); //add the amount of memory that is not being used to the memoryLoss variable
				}
	...
	private static void print(Queue<Job> output, int timeSeg, BufferedWriter w) //this function will write the contents.  Should be able to copy/paste it.
    {
	    if(!output.isEmpty())
